...
// Funkcja bada cyklicznoœæ sk³adowej grafu
//-----------------------------------------
bool isComponentCyclic(slistEl ** G, int v, bool * visited)
{
  ...
  while(!S.empty())             // W pêtli przechodzimy graf za pomoc¹ DFS
  {
    ...
    for(slistEl *p = G[v]; p; p = p->next) // Przegl¹damy listê s¹siadów
    {
      z = p->v;                 // Numer s¹siada
      if(!visited[z])
      {
...
bool isCyclic(int n, slistEl ** G)
{
  ...
  for(i = 0; i < n; i++)
    visited[i] = false;         // Tablicê zerujemy
  for(i = 0; i < n; i++)        // Szukamy wierzcho³ka startowego
    if(!visited[i] && isComponentCyclic(G,i,visited))
    {
      delete [] visited;
      return true;              // Cykl znaleziony, koñczymy z true
    }
  delete [] visited;
  return false;                 // Graf nie posiada cyklu, koñczymy z false
}
